...
def update_game():
    ...
    if not cheat_mode and current_time < brick_fall_stop_end_time:
        pass
    elif not cheat_mode and current_time - brick_spawn_time > BRICK_SPAWN_INTERVAL:
        brick_x, brick_y = 0, 0
        rand_num = random.uniform(0, 100)
        
        # MD SAIDUL ISLAM APU: Random brick fall on myself and life decrease
        if rand_num < 70:  # 70% of the time, fall on the player
            brick_x = player_pos[0]
            brick_y = player_pos[1]
        elif rand_num < 90:  # 20% of the time, fall around the player
            brick_x = player_pos[0] + random.uniform(-50, 50)
            brick_y = player_pos[1] + random.uniform(-50, 50)
        else:  # 10% of the time, fall anywhere
            brick_x = random.uniform(-GRID_LENGTH, GRID_LENGTH)
            brick_y = random.uniform(-GRID_LENGTH, GRID_LENGTH)
        
        random_bricks.append({
            'pos': [
                brick_x,
                brick_y,
                100.0
    ...
    for treasure in treasures[:]:
        ...
        if math.hypot(dx, dy) < 20:
            if treasure['special'] and (has_special_key or cheat_mode):
                ...
                # MD SAIDUL ISLAM APU: Collecting treasure + eating special treasure gives 3 points
                score += 3 # Special treasure gives 3 points
                player_speed += 0.5
